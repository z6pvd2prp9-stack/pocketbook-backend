...
// Teller requires certificate-based auth
const tlsOptions = process.env.TELLER_CERT && process.env.TELLER_KEY ? {
...
app.get("/health", (_, res) => res.json({ status: "ok", provider: "teller", hasCert: !!tlsOptions }));
  ...
  try {
    ...
    const accounts = await Promise.all(r.data.map(async (a) => {
      ...
      try {
        ...
        if (br.status === 200) {
          balance = parseFloat(br.data.available || br.data.ledger || 0);
...
app.listen(PORT, () => console.log("PocketBook backend running on port", PORT, tlsOptions ? "(with TLS cert)" : "(no cert - sandbox only)"));
